...
std::string output_path = "MTMCT\\";

bool firstTime = true;
bool menuActive = false;
bool fly = false;

bool recordSegmentation = false;
...
void firstOpen()
{
	...
	PLAYER::SET_SWIM_MULTIPLIER_FOR_PLAYER(mainPlayer, 5.0f);
	PLAYER::SET_RUN_SPRINT_MULTIPLIER_FOR_PLAYER(mainPlayer, 10.49f);
...
void FunctionToolbox::stopControl()
{
	if (stopCoords) {
...
void FunctionToolbox::listen_for_keystrokes() {
	/**
...
void FunctionToolbox::cameraCoords()
{
	char text[100];
...
void FunctionToolbox::main_menu()
{
	const float lineWidth = 250.0;
	const int lineCount = 8;
	menuActive = true;

	std::string caption = "Func. Toolbox";
...
void FunctionToolbox::peds_menu()
{
	const float lineWidth = 250.0;
...
void FunctionToolbox::camera_menu()
{
	const float lineWidth = 250.0;
...
void FunctionToolbox::place_menu()
{
...
void FunctionToolbox::time_menu()
{
	const float lineWidth = 250.0;
...
void FunctionToolbox::weather_menu()
{
	const float lineWidth = 250.0;
...
void FunctionToolbox::resetMenuCommands()
{
	bEnter = false;
	bBack = false;
	bUp = false;
...
void FunctionToolbox::tasks_sub_menu()
{
...
void FunctionToolbox::cancelLastLog() {
...
void FunctionToolbox::spawn_peds(Vector3 pos, int num_ped) {
	...
	for (int i = 0; i<num_ped; i++) {
		...
		STREAMING::REQUEST_MODEL(1358380044);
		while (!STREAMING::HAS_MODEL_LOADED(1358380044)) {
			WAIT(1);
		}

		ped[i] = PED::CREATE_PED(10, 1358380044, pos.x, pos.y, pos.z, 0.0f, false, true);

		PED::SET_PED_RANDOM_PROPS(ped[i]);
...
void FunctionToolbox::walking_peds()
{
...
void FunctionToolbox::file_menu()
{
	const float lineWidth = 250.0;
	const int lineCount = 4;
	menuActive = true;
...
void FunctionToolbox::draw_text(char *text, float x, float y, float scale) {
	UI::SET_TEXT_FONT(0);
